...
// flags
__constant__ __device__ int devDecomposedResults = 0;
__constant__ __device__ int devLDResults = 0;
...
__host__ void copyConstantsToDevice(
        ...
        int decomposedResults = decomposed? 1 : 0;
        checkCUDAError(cudaMemcpyToSymbol(devDecomposedResults, &decomposedResults, sizeof(decomposedResults), 0, cudaMemcpyHostToDevice))

        int ldResults = ld? 1 : 0;
        checkCUDAError(cudaMemcpyToSymbol(devLDResults, &ldResults, sizeof(ldResults), 0, cudaMemcpyHostToDevice))
...
__device__ __host__ void generateContingencyTable2Way(uint32_t *dest, uint32_t *numcases, uint32_t *numctrls,
        const unsigned char* db, unsigned snpsize, unsigned casecount, unsigned a, unsigned b) {
    ...
    for(unsigned pos = 0; pos < snpsize; pos++) {
        ...
        unsigned char gta = db[a*snpsize + pos] & 3;
        unsigned char gtb = db[b*snpsize + pos] & 3;
        ...
        gta = (db[a*snpsize + pos] >> 2) & 3;
        gtb = (db[b*snpsize + pos] >> 2) & 3;
        counter = gta * 3 + gtb;
        if(gtpos < casecount)
            counter += 9;
        if (gta != 3 && gtb != 3) // do not count unknowns
        ...
        gta = (db[a*snpsize + pos] >> 4) & 3;
        gtb = (db[b*snpsize + pos] >> 4) & 3;
        counter = gta * 3 + gtb;
        if(gtpos < casecount)
            counter += 9;
        if (gta != 3 && gtb != 3) // do not count unknowns
        ...
        gta = (db[a*snpsize + pos] >> 6) & 3;
        gtb = (db[b*snpsize + pos] >> 6) & 3;
        counter = gta * 3 + gtb;
        if(gtpos < casecount)
            counter += 9;
        if (gta != 3 && gtb != 3) // do not count unknowns
...
__device__ __host__ void generateContingencyTable3Way(uint32_t *dest, uint32_t *numcases, uint32_t *numctrls,
        const unsigned char* db, unsigned snpsize, unsigned casecount, unsigned a, unsigned b, unsigned c) {
    ...
    for(unsigned pos = 0; pos < snpsize; pos++) {
        unsigned char gta = db[a*snpsize + pos] & 3;
        unsigned char gtb = db[b*snpsize + pos] & 3;
        unsigned char gtc = db[c*snpsize + pos] & 3;
...
//        assert(result.getScoreFields() >= 4);
        ...
        result.setScore(s++, ldAB);
        result.setScore(s++, ldAC);
        result.setScore(s++, ldBC);
    }
...
//        assert(result.getScoreFields() >= 3);
        ...
        result.setScore(s++, hxy);
        result.setScore(s++, hx);
    }
}

__global__ void MutualInformationKernel3Way(uint16_t *values,
                                        size_t tablesExpected,
    ...
    generateContingencyTable3Way(ctable, &numCases, &numControls, db, devSNPSize, devCaseCount, a, b, c);

    MutualInformationKernel3WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
__global__ void MutualInformationKernel2Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    unsigned a = ((unsigned*)values)[0];
    unsigned b = ((unsigned*)values)[1];
    generateContingencyTable2Way(ctable, &numCases, &numControls, db, devSNPSize, devCaseCount, a, b);

    MutualInformationKernel2WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
//        assert(result.getScoreFields() >= 8);
        ...
        result.setScore(s++, mi3);
        result.setScore(s++, mi2a);
        result.setScore(s++, mi2b);
        result.setScore(s++, mi2c);
        result.setScore(s++, mi1a);
        result.setScore(s++, mi1b);
        result.setScore(s++, mi1c);
    }
}

__global__ void InformationGainKernel3Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    InformationGainKernel3WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
//        assert(result.getScoreFields() >= 4);
        ...
        result.setScore(s++, mi2);
        result.setScore(s++, mi1a);
        result.setScore(s++, mi1b);
    }
}

__global__ void InformationGainKernel2Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    InformationGainKernel2WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
__device__ __host__ void BOOSTKernel2WayCore(const uint32_t *ctable,
    ...
    // Filter
    GPUEngine::score_type valueFilterKSASA = 0;
...
//        }
    ...
    result.setID(0, id[SNP_A]);
    result.setID(1, id[SNP_B]);
    result.setScore(0, valueFilter);
    result.setScore(1, valueError);
    result.setScore(2, (valueFilter > 0 ? approx_p_4df(valueFilter) : 1.0)); // approximate p-value assuming a chi-squared distribution with 4 df

    int s = 3;
    if (ld) {
//        assert(result.getScoreFields() >= 5);
        double rsq_high, rsq_low;
        calcLD_2way(cases, controls, numCases, numControls, &rsq_high, &rsq_low); //, id[SNP_A], id[SNP_B]);
        result.setScore(s++, rsq_high);
        result.setScore(s++, rsq_low);
    ...
    if (decomp) {
//        assert(result.getScoreFields() >= 6);
        result.setScore(s++, (GPUEngine::score_type) valueIter);
        result.setScore(s++, valueFilterKSA);
        result.setScore(s++, valueFilterKSASA);
    }


}

__global__ void BOOSTKernel2Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    BOOSTKernel2WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
__device__ __host__ void LogLinearKernel2WayCore(const uint32_t *ctable,
    ...
    result.setScore(0, valueFilter);
    result.setScore(1, valueError);
    result.setScore(2, approx_p_4df(valueFilter)); // approximate p-value assuming a chi-squared distribution with 4 df
...
__global__ void LogLinearKernel2Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    LogLinearKernel2WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
__device__ __host__ void LogisticRegressionKernel2WayCore(const uint32_t *ctable,
    ...
    result.setScore(0, returncode == 0? zsq : 0);
    result.setScore(1, returncode == 0? oddr : 0);
    result.setScore(2, returncode == 0? approx_p_1df(zsq) : 1.0); // calculate approximate p-value (assuming chi-square with 1 df)
    result.setScore(3, returncode == 0? beta[3] : 0);

    int s = 4;
    if (ld) {
//        assert(result.getScoreFields() >= 6);
        double rsq_high, rsq_low;
        calcLD_2way(&ctable[9], ctable, numCases, numControls, &rsq_high, &rsq_low); //, id[SNP_A], id[SNP_B]);
        result.setScore(s++, rsq_high);
        result.setScore(s++, rsq_low);
    }
    ...
    if (decomp) {
//        assert(result.getScoreFields() >= 8);
        result.setScore(s++, (GPUEngine::score_type) iteration);
        result.setScore(s++, delta);
        result.setScore(s++, min_delta);
        result.setScore(s++, (GPUEngine::score_type) returncode);
    }

}

__global__ void LogisticRegressionKernel2Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    ...
    LogisticRegressionKernel2WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
...
__device__ __host__ void LogisticRegressionKernel3WayCore(const uint32_t *ctable,
    ...
    result.setScore(1, returncode == 0? oddr : 0);
    result.setScore(2, returncode == 0? approx_p_1df(zsq) : 1.0); // calculate approximate p-value (assuming chi-square with 1 df)
    result.setScore(3, returncode == 0? beta[7] : 0);

    int s = 4;
    if (ld) {
//        assert(result.getScoreFields() >= 7);
        double ldAB, ldAC, ldBC;
        calcPairwiseLD_3way(&ctable[27], ctable, numCases, numControls, &ldAB, &ldAC, &ldBC); //, id[SNP_A], id[SNP_B], id[SNP_C]);
        result.setScore(s++, ldAB);
        result.setScore(s++, ldAC);
        result.setScore(s++, ldBC);
    }
...
__global__ void LogisticRegressionKernel3Way(uint16_t *values,
                                        size_t tablesExpected,
                                        ResultView<> resultView) {
    const unsigned char* db = (unsigned char*)(values + devDBOffset/sizeof(*values)); // only required for GPU only run
    ...
    LogisticRegressionKernel3WayCore(ctable, numCases, numControls, resultView.getDeviceResult(gid), devLDResults, devDecomposedResults);
}
